...
**Next direction:** atom-creation hardening on the `decode_structs: true` path is the leader of the remaining standalone work. `function` and `fixed<M>x<N>` / `ufixed<M>x<N>` implementations both score ⚠️ poor ROI and are deferred unless user demand surfaces. Public-surface feature parity with `eth-abi` / `ethers` / `viem` / `alloy` is now substantially closed.

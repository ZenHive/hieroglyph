...
- [exthereum/abi#54](https://github.com/exthereum/abi/issues/54) — `fixed`/`ufixed`/`function` parse-but-don't-encode + `@type type` gaps. Filed 2026-04-24. Fork fix shipped on `zenhive` (parse-time rejection in `ABI.Parser` + `{:bytes, N}` added to `@type type`); awaiting upstream response. A related `fixed<M>x<N>` lexer sub-bug (single `x` shadowed by LETTERS rule) was discovered during test-writing — local fix shipped in 1.2.0 (dedicated `fixed_typename`/`ufixed_typename` terminals + `'x'`-rule reorder + `identifier_part` extension); upstream filing deferred — will be batched into a future combined-bugs issue with PR offer.
